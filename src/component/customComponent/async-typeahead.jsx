...
export default class TAsyncTypeahead extends React.Component {
  ...
  _handleRequestItem = (keyword, page = 1) =>
    this.props.onRequestItem
      ? this.props.onRequestItem(keyword, page, this.state.maxResults)
      : { options: [], total: 0 };
  ...
  _onPaginate = (e, shownResults) => {
    ...
    if (
      _cache[keyword].options.length > shownResults ||
      _cache[keyword].options.length === _cache[keyword].total
    )
      return;
  ...
  _renderItem = (item, props, idx) =>
    this.props.renderItem ? this.props.renderItem(item, props, idx) : <div key={idx}>{item.key}</div>;
